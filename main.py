...
class BaseLanguageModel:
    ...
    def _parse_response(response: str) -> DeepseekOutput:
        ...
        try:
            ...
            print(f"Warning: Could not parse any valid fields from response:\n{response}")
        ...
        for attempt in range(self.max_retries):
            try:
                completion = self.client.chat.completions.create(
                    ...
                    temperature=self.config.temperature + (attempt * 0.1),  # Slightly increase temperature on retries
